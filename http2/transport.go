...
import (
	...
	"github.com/lingulingo/tlsprint/http2/internal/httpcommon"
...
type Transport struct {
	...
	// Fingerprint, when set, customises the HTTP/2 connection fingerprint
	// (SETTINGS preamble, WINDOW_UPDATE, header order, stream priority).
...
// Fingerprint customises the HTTP/2 connection-level fingerprint emitted by
// the Transport: the SETTINGS preamble, the initial connection
// WINDOW_UPDATE, the request pseudo/header order, and the priority applied to
// request streams. When nil, the upstream default fingerprint is used.
type Fingerprint struct {
	// Settings is the exact ordered SETTINGS payload sent in the client
	// connection preface. When empty, the Transport's default settings are
	// used.
	Settings []Setting
	// WindowUpdate is the connection WINDOW_UPDATE increment sent after the
	// SETTINGS frame. When zero, the Transport default is used.
	WindowUpdate uint32
	// PseudoHeaderOrder sets the request pseudo-header order (short codes
	// "m"/"a"/"s"/"p" or full names).
	PseudoHeaderOrder []string
	// HeaderOrder sets the request header send order; request headers not
	// listed are appended after.
	HeaderOrder []string
	// HeaderPriority, when set, attaches stream priority to the request
	// HEADERS frame.
	HeaderPriority *StreamPriority
	// PriorityFrames are explicit PRIORITY frames sent after the connection
	// preface.
...
// StreamPriorityFrame is an explicit PRIORITY frame.
type StreamPriorityFrame struct {
	StreamID uint32
...
func (t *Transport) newClientConn(c net.Conn, singleUse bool, internalStateHook func()) (*ClientConn, error) {
	...
	if t.Fingerprint != nil && len(t.Fingerprint.Settings) > 0 {
		cc.fr.WriteSettings(t.Fingerprint.Settings...)
	} else {
		cc.fr.WriteSettings(initialSettings...)
	}
	if t.Fingerprint != nil && t.Fingerprint.WindowUpdate != 0 {
		cc.fr.WriteWindowUpdate(0, t.Fingerprint.WindowUpdate)
	} else {
		cc.fr.WriteWindowUpdate(0, uint32(conf.MaxUploadBufferPerConnection))
	}
	if t.Fingerprint != nil {
		for _, pf := range t.Fingerprint.PriorityFrames {
			cc.fr.WritePriority(pf.StreamID, PriorityParam{
...
func (cs *clientStream) encodeAndWriteHeaders(req *http.Request) error {
	...
	cc.hbuf.Reset()
	res, err := encodeRequestHeaders(req, cs.requestedGzip, cc.peerMaxHeaderListSize, cc.t.Fingerprint, func(name, value string) {
	...
	cs.sentHeaders = true
	err = cc.writeHeaders(cs.ID, endStream, int(cc.maxFrameSize), hdrs, cc.t.Fingerprint)
	traceWroteHeaders(cs.trace)
	return err
}

func encodeRequestHeaders(req *http.Request, addGzipHeader bool, peerMaxHeaderListSize uint64, fp *Fingerprint, headerf func(name, value string)) (httpcommon.EncodeHeadersResult, error) {
...
func (cc *ClientConn) writeHeaders(streamID uint32, endStream bool, maxFrameSize int, hdrs []byte, fp *Fingerprint) error {
	...
	for len(hdrs) > 0 && cc.werr == nil {
		...
		if first {
			...
			if fp != nil && fp.HeaderPriority != nil {
				param.Priority = PriorityParam{
					StreamDep: fp.HeaderPriority.StreamDep,
					Exclusive: fp.HeaderPriority.Exclusive,
					Weight:    fp.HeaderPriority.Weight,
				}
			}
...
func (cs *clientStream) writeRequestBody(req *http.Request) (err error) {
	...
	if len(trls) > 0 {
		err = cc.writeHeaders(cs.ID, true, maxFrameSize, trls, cc.t.Fingerprint)
	} else {
