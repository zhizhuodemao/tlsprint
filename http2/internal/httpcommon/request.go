...
type EncodeHeadersParam struct {
	...
	// PseudoHeaderOrder, when set, overrides the order of the standard
	// request pseudo-headers. Values may be the short codes used by
	// fingerprint services ("m"/"a"/"s"/"p") or the full names
	// (":method"/":authority"/":scheme"/":path"). When empty, the upstream
	// order (authority, method, path, scheme) is used.
	PseudoHeaderOrder []string

	// HeaderOrder, when set, specifies the order regular headers are sent.
	// Headers present in the request but not listed are appended afterwards.
	// When empty, headers are sent in map order.
	HeaderOrder []string
}
...
func EncodeHeaders(ctx context.Context, param EncodeHeadersParam, headerf func(name, value string)) (res EncodeHeadersResult, _ error) {
	...
	enumerateHeaders := func(f func(name, value string)) {
		...
		if len(param.PseudoHeaderOrder) > 0 && !isNormalConnect {
			codes := map[string]string{"m": ":method", "a": ":authority", "s": ":scheme", "p": ":path"}
			for _, tok := range param.PseudoHeaderOrder {
				name := tok
				if full, ok := codes[strings.ToLower(tok)]; ok {
					name = full
				}
				switch name {
				case ":authority":
		...
		if len(param.HeaderOrder) > 0 {
			...
			for _, name := range param.HeaderOrder {
				c := textproto.CanonicalMIMEHeaderKey(name)
				if _, ok := req.Header[c]; !ok || seen[c] {
					continue
				}
				seen[c] = true
				hkeys = append(hkeys, c)
			}
